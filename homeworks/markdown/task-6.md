...
Цель домашнего задания подготовить инфраструктуру для будущего проекта и научиться настраивать веб-серверы.

## 2. Срок сдачи

ДЗ5 и ДЗ6 нужно сдать к экзамену. Работы, отправленные на проверку позже, оцениваются в половину максимальных баллов на пересдаче.
...
## 3. Настройка gunicorn для запуска WSGI скриптов
...
## 4. Создание простого WSGI-скрипта
...
Настройка nginx для отдачи статического контента - 5 баллов
- общее - 3
- локейшен /uploads/ приоритетнее статики - 1
- проксирование - 1

Настройка gunicorn для запуска wsgi приложений - 2 балла
- общее - 2

Создание WSGI-приложения - 2 балла
- наличие запускаемого скрипта - 1
- правильное отображение списка GET и POST параметров - 1

Оценка производительности nginx и gunicorn - 5 баллов
